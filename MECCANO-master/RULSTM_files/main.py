...
from meccano_dataset import MeccanoSequenceDataset
...
parser.add_argument('--modality', type=str, default='rgb',
                    choices=['rgb', 'flow', 'obj', 'depth', 'gaze', 'hands', 'fusion'], help = "Modality. Rgb/flow/obj/depth/gaze/hands represent single branches, whereas fusion indicates the whole model with modality attention.")
...
parser.add_argument('--feats_in', type=int, nargs='+', default=[20, 20, 20],
                    help='Input sizes when the fusion modality is selected. MECCANO features: Obj/Gaze/Hands size=20, RGB/Depth size=2304')
...
parser.add_argument('--meccanomulti', action='store_true',
                    help="Whether to use MECCANO-MULTIMODAL")
...
def get_loader(mode, override_modality = None):
    if override_modality:
        ...
        #Here you can choose the branches that you want to use
        path_to_lmdb = join(args.path_to_data, args.modality) if args.modality != 'fusion' else [join(args.path_to_data, m) for m in ['obj','gaze','hands']]
    ...
    _set = MeccanoSequenceDataset(**kargs)
...
def get_model():
    if args.modality != 'fusion':  # single branch
        ...
        obj_model = RULSTM(args.num_class, args.feats_in[0], args.hidden, args.dropout, return_context = args.task=='anticipation')
        gaze_model = RULSTM(args.num_class, args.feats_in[1], args.hidden, args.dropout, return_context = args.task=='anticipation')
        hands_model = RULSTM(args.num_class, args.feats_in[2], args.hidden, args.dropout, return_context = args.task=='anticipation')
        

        if args.task=='early_recognition' or (args.mode == 'train' and not args.ignore_checkpoints):
            print(exp_name.replace('fusion','rgb') +'_best.pth.tar')
           
           # Load the checkpoints related to the chosen branches
            checkpoint_obj = torch.load(join(args.path_to_models,\
                    exp_name.replace('fusion','obj') +'_best.pth.tar'))['state_dict']
            checkpoint_gaze = torch.load(join(args.path_to_models,\
                    exp_name.replace('fusion','gaze') +'_best.pth.tar'))['state_dict']
            checkpoint_hands = torch.load(join(args.path_to_models,\
                    exp_name.replace('fusion','hands') +'_best.pth.tar'))['state_dict']


            obj_model.load_state_dict(checkpoint_obj)
            gaze_model.load_state_dict(checkpoint_gaze)
            hands_model.load_state_dict(checkpoint_hands)
        
        if args.task == 'early_recognition':
            return [obj_model, gaze_model, hands_model]

   
        model = RULSTMFusion([obj_model, gaze_model, hands_model], args.hidden, args.dropout)
...
def main():
    ...
    if args.mode == 'train':
        ...
        if args.task == 'anticipation' and args.meccanomulti:
            ...
            print(
                f"\nMean TtA(5): ACTION: {tta_action:0.2f}")
