...
class MeccanoSequenceDataset(data.Dataset):
    def __init__(self, path_to_lmdb, path_to_csv, label_type = 'action',
                time_step = 0.25, sequence_length = 14, fps = 12,
                img_tmpl = "{:05d}.jpg",
                vid_tmpl = "{:02d}",
                transform = None,
                challenge = False,
    ...
    def __get_frames(self, frames, video):
        ...
        frames = np.array(list(map(lambda x: self.vid_tmpl.format(video)+"_"+self.img_tmpl.format(x), frames)))
        return frames
